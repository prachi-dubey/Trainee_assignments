$(document).ready(function(){
  ...
  function prepareCalendar (date) {
    ...
    var checkYear = new Date().getFullYear();                    //give month currently
    var currentDate = date.getDate();
    var currentMonth = date.getMonth();
    var currentYear = date.getFullYear();
    $("#curr-month").val(currentMonth);
    $("#curr-year").val(currentYear);
    ...
    for(var i = 1; i <= totalDate; i++) {
      if(currentDate == i && currentMonth == checkMonth && currentYear == checkYear) {                                  //foucs current date
